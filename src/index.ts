...
import {generateJSONRPCService} from "./service/jsonrpc";
...
withAllStdIn((inputBuff: Buffer) => {
  try {
    ...
    codeGenRequest.getFileToGenerateList().forEach(fileName => {
      ...
      if (generateServices) {
        generateJSONRPCService(outputFileName, fileNameToDescriptor[fileName], exportMap)
    ...
    console.error("angular-jrpc-proto-gen error: " + err.stack + "\n");
    process.exit(1);
  }
});
